...
fn get_or_create_active_workspace(app_handle: &AppHandle) -> Result<std::path::PathBuf, String> {
    ...
    if let Some(ws_id) = active_id {
        project_storage::get_workspace_dir(app_handle, &ws_id)
    } else {
        ...
        project_storage::get_workspace_dir(app_handle, &info.workspace_id)
    }
}
...
async fn receive_log(Json(payload): Json<LogRequest>) -> Json<GenericResponse> {
    println!(
        "API CALLED: /api/log: [{}][{}] {}",
        payload.source, payload.level, payload.message
    );
    Json(GenericResponse {
        success: true,
        error: None,
    ...
    if let Ok(entries) = std::fs::read_dir(ws_dir) {
        for entry in entries.filter_map(Result::ok) {
            if let Ok(file_type) = entry.file_type() {
                if file_type.is_file() {
                    if let Some(name) = entry.file_name().to_str() {
                        if !name.starts_with('.') && name != project_storage::PROJECT_STATE_FILENAME
                        {
                            files.push(name.to_string());
                        }
                    }
