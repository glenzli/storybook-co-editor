...
- **Codex 是可选的本机依赖。** 若要使用 Codex 润色、Codex 绘制或 Codex 重绘，请安装并登录 Codex / ChatGPT 桌面端，使其提供本机 Codex App Server。应用启动时会读取可用模型；未检测到可用 Codex 时，这些入口会禁用，编辑、导出和 MCP 仍可独立使用。
...
- 新建页面菜单可选空白页或 `使用 Codex 绘制`。新绘制会按当前画布比例生成候选图，确认后才会追加为新页面。
- 页面样式区顶部、缩略图右键菜单和中心画布右键菜单均提供 `Codex 重绘图片`。重绘时应用把当前图片的隔离 PNG 副本作为输入，调用 Codex 的 `imagegen` 工具生成候选图；候选图在确认前不会改变页面，也不会打包进 `.scproj`，确认后才替换当前页并保留原图至回收站。
...
MCP 服务只监听 `127.0.0.1:14320`，并使用首次启动时生成的本机令牌。令牌拥有当前项目的读取和编辑权限，不应写入仓库或交给不受信任的客户端；轮换令牌后需要重新安装 Codex 条目。图像像素不会作为 MCP 资源暴露；第一版只提供项目结构、剧本和少量排版参数。Codex 绘制和重绘由桌面应用独立发起：新绘制会向所选模型发送绘制要求，重绘会额外发送当前图片的隔离副本和重绘要求。
