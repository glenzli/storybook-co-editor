...
- 新建页面菜单可选空白页或 `使用 Codex 绘制`；候选图经确认后才会追加为新页面。页面样式区顶部、缩略图右键菜单和中心画布右键菜单均提供 `Codex 重绘图片`。重绘时应用把隔离副本作为图片输入，调用 Codex 的 `imagegen` 工具生成候选图；候选图在确认前不会改变页面，确认后才替换当前页并保留原图至回收站。
